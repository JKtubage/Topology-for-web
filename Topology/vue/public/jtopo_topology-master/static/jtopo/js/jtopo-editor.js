...
TopologyPanel.prototype.saveTopology = function () {
    ...
    $.ajax({
        type: "POST",
        url: "http://localhost:3000/saveTopology",
        data: containers ,
 ...
 * @param backImg 拓扑图的背景图片
 */
TopologyPanel.prototype.ByJson = function (topologyJson, backImg) {
    try {
    ...
    self.nodeMenu.on('click', function (event) {
        ...
        switch (text) {
            ...
            case '逆时针旋转(Shift+I)':
                ...
                break
            case '节点设置':
                console.log(editor.scene.selectedElements[0].selectedLocation)
                //console.log(editor.stage)
               // console.log(editor.scene)
                var a = editor.stage.find('node')
                console.log(a)
                editor.scene.selectedElements[0].fillStyle = 'rgba(255,255,0,0.5)'
                a['asd'] = 1
                console.log(a.asd)
                self.setting.show()
                c = new JTopo.Tip()
...
// //梯形图点击创建节点
// TopologyEditor.prototype.creatsign = function{
//     this.sign.click(function(e){
//         switch()
//     })
// }
/**
...
editor.utils = {
    ...
    setEditMode: function () {
        console.log(editor.stage)
        let blob = new Blob([editor.stage.toJson()], {type: "application/json"});
        let url = URL.createObjectURL(blob);
        
        let a = document.createElement('a');
        a.href = url;
        a.download = "topology.json"; // 你想要的文件名
